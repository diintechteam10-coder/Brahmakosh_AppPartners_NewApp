...
class AshtakvargaTab extends StatefulWidget {
  final AshtakvargaData? ashtakvargaData;
  final String? ascendantSign;

  const AshtakvargaTab({
    super.key,
    required this.ashtakvargaData,
    this.ascendantSign,
  });
...
class _AshtakvargaTabState extends State<AshtakvargaTab> {
  ...
  Widget build(BuildContext context) {
    if (widget.ashtakvargaData == null ||
        widget.ashtakvargaData!.rows.isEmpty) {
  ...
  Widget _buildChart() {
    // We're calculating the sum of total points across all planets per sign for the chart
    final Map<int, int> housePointsMap = {};
    int ascNum = _getSignNumber(widget.ascendantSign);

    final Map<int, int> signTotalPoints = {};
    for (var row in widget.ashtakvargaData!.rows) {
      int signNum = _getSignNumber(row.sign);
      int total = row.su + row.mo + row.ma + row.me + row.ju + row.ve + row.sa;
      signTotalPoints[signNum] = total;
    }
  ...
  Widget _buildTable() {
    final factors = ['su', 'mo', 'ma', 'me', 'ju', 've', 'sa'];
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            // Body rows
            ...widget.ashtakvargaData!.rows.map((row) {
              int total =
                  row.su + row.mo + row.ma + row.me + row.ju + row.ve + row.sa;
              return Column(
                children: [
                  Row(
                    children: [
                      _buildCell(
                        _formatSignName(row.sign),
                        width: 80,
                        isHeader: true,
                        align: TextAlign.start,
                        padding: const EdgeInsets.only(left: 8),
                      ),
                      _buildCell(row.su.toString(), width: 40),
                      _buildCell(row.mo.toString(), width: 40),
                      _buildCell(row.ma.toString(), width: 40),
                      _buildCell(row.me.toString(), width: 40),
                      _buildCell(row.ju.toString(), width: 40),
                      _buildCell(row.ve.toString(), width: 40),
                      _buildCell(row.sa.toString(), width: 40),
                      _buildCell(
                        total.toString(),
                        width: 50,
                        isBold: true,
